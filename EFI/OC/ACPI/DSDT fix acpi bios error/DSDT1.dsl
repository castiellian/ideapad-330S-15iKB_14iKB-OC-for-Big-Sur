 ...
 *
 * Disassembly of iASL8aQb9u.aml, Mon Jan 25 23:32:05 2021
 *
 * Original Table Header:
 *     Signature        "DSDT"
 *     Length           0x0001D7B4 (120756)
 *     Revision         0x02
 *     Checksum         0x9B
 *     OEM ID           "LENOVO"
 *     OEM Table ID     "CB-01   "
...
DefinitionBlock ("", "DSDT", 2, "LENOVO", "CB-01   ", 0x00000001)
{
    ...
    External (_SB_.PCI0.GFX0.AFN2, MethodObj)    // 1 Arguments (from opcode)
    ...
    Name (SS3, One)
    Name (SS4, One)
    OperationRegion (GNVS, SystemMemory, 0x8CF6A018, 0x0767)
    ...
    OperationRegion (OGNS, SystemMemory, 0x8CF6EF18, 0x90)
    ...
    Scope (_SB.PCI0.LPCB)
    {
        Device (H_EC)
        {
            ...
            Device (BAT0)
            {
                ...
                Method (XBIF, 0, NotSerialized)
                {
                    ...
                    If (ECON)
                    {
                        ...
                        Store (Multiply (BDC0, BASC), Index (BPKG, One))
                        Store (Multiply (FCC0, BASC), Index (BPKG, 0x02))
                        Store (BDV0, Index (BPKG, 0x04))
                        If (FCC0)
                        {
                            Store (Divide (Multiply (FCC0, BASC), 0x0A, ), Index (BPKG, 0x05))
                            Store (Divide (Multiply (FCC0, BASC), 0x19, ), Index (BPKG, 0x06))
                            Store (Divide (Multiply (BDC0, BASC), 0x64, ), Index (BPKG, 0x07))
                        }
                ...
                Method (XBST, 0, NotSerialized)
                {
                    ...
                    If (ECON)
                    {
                        ...
                        {
                            Sleep (0x14)
                            Add (And (Not (BAC0), 0xFFFF), One, Local0)
                        }
                        Else
                        {
                            Sleep (0x14)
                        ...
                        If (LEqual (Local1, 0x8000))
                        {
                            Divide (Multiply (Local0, BAV0), 0x03E8, , Local0)
                        }

                        Store (Local0, Index (PKG1, One))
                        Sleep (0x14)
                        Store (Multiply (BRC0, BASC), Index (PKG1, 0x02))
                        Store (BAV0, Index (PKG1, 0x03))
                    }
            ...
            Device (LID0)
            {
                ...
                Method (_PRW, 0, NotSerialized)  // _PRW: Power Resources for Wake
                {
                    Return (XPRW (0x0E, 0x03))
                }

                Method (XLID, 0, NotSerialized)
                {
    ...
    OperationRegion (MDBG, SystemMemory, 0x8CF57018, 0x1004)
